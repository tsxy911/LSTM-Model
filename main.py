...
frame = "keras"  # 可选： "keras", "pytorch", "tensorflow"
if frame == "pytorch":
...
class Config:
    # 数据参数
    feature_columns = list(range(1, 3))     # 要作为feature的列，按原数据从0开始计算，也可以用list 如 [2,4,6,8] 设置
    ...
    batch_size = 32
    learning_rate = 0.001
    epoch = 1500                  # 整个训练集被训练多少遍，不考虑早停的前提下
...
class Data:
    def __init__(self, config):
        ...
        self.start_num_in_test = 21      # 测试集中前几天的数据会被删掉，因为它不够一个time_step
